...
def infer_ps_sign_as_load(df_train: pd.DataFrame) -> int:
    """Infer robust sign convention for pumped-storage.

    Goal: create PS_as_load that behaves like *additional demand*:
      - PS_as_load > 0 tends to increase RT (all else equal)
      - PS_as_load < 0 tends to decrease RT

    We infer sign from correlation between PS_act and RT on the training set.
    If corr(PS_act, RT) >= 0 => treat PS_act as already 'as_load' (sign=+1).
    Else => flip sign (sign=-1).

    Returns:
        ps_sign_as_load in {+1, -1}
    """
    tmp = df_train.dropna(subset=["PS_act", COL_RT]).copy()
    if len(tmp) < 500:
        # fall back to +1 if too few points
        return 1
    corr = float(tmp["PS_act"].corr(tmp[COL_RT]))
    if np.isnan(corr):
        return 1
...
def train_and_save(history_path: str, model_dir: str, th_spike: float, th_neg: float) -> None:
    ...
    df_b2["PS_pred_as_load"] = float(ps_sign_as_load) * df_b2["PS_p50_oof"]
...
def predict_one_day(
    ...
    if ps_models is not None:
        ...
        df_fc["PS_pred_as_load"] = float(ps_sign_as_load) * df_fc["PS_pred_p50"]
...
def run_without_args() -> None:
    CONFIG = {
        ...
        "arm_day_neg": 0.40,
        # predict
        "forecast": r"forecast_input_2026-01-07.xlsx",
        "date": "2026-01-07",
        "out": r"outPut/forcast_2026-01-07.xlsx",
        "trading_brief": 1,
        "trading_out": r"",


        # backtest
        "dates": "2026-01-01, 2026-01-02, 2026-01-03, 2026-01-04, 2026-01-05, 2025-03-21, 2025-03-20, 2025-03-19, 2025-03-18, 2025-05-24,2025-04-21, 2025-10-01, 2025-05-01, 2024-03-28, 2024-05-11, 2024-01-15, 2024-05-03, 2024-05-02, 2024-02-14,2025-02-01, 2025-01-31, 2025-02-02, 2024-05-17, 2024-05-18, 2025-07-22, 2025-07-21, 2025-12-02, 2025-12-09, 2025-11-06",
        "report_out": r"outPut/eval_backtest_v2_6.xlsx",
    }
