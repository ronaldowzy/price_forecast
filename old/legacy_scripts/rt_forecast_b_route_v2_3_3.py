...
def predict_one_day(
    ...
    # Day-level arming for negative gate: avoid false triggering on days where p_neg is only "moderately" high.
    armed_neg = True
    if arm_day_neg is not None and float(arm_day_neg) > 0:
        armed_neg = bool(np.nanmax(p_neg) >= float(arm_day_neg))
    p_neg_eff = p_neg if armed_neg else np.zeros_like(p_neg)

    # gate outputs (priority: neg -> spike -> normal)
    take_neg = p_neg_eff >= p_gate_neg
    take_spike = (~take_neg) & (p_spike >= p_gate_spike)

    rt_gate_p50 = np.where(take_neg, rtG50, np.where(take_spike, rtS50, rtN50))
...
def backtest_dates(
    ...
    for d in dates:
        ...
        predict_one_day(history_path, model_dir, tmp_fc_path, d, tmp_pred_path, p_gate_spike=p_gate_spike, p_gate_neg=p_gate_neg, arm_day_neg=arm_day_neg)
