...
def add_within_day_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # For the first few points in a day, diff() yields NaN.
    # V1.1 decision: fill them with 0 so we keep all 96 slots in prediction output.
...
def predict_one_day(history_path: str, model_dir: str, forecast_path: str, date_str: str, out_path: str) -> None:
    ...
    # If error-profile stats are missing (e.g., short history), use neutral 0.0 instead of dropping slots.
    ...
    # Ensure lags are numeric (they may have been filled by fallbacks)
    for c in ["RT_lag_1d", "RT_lag_7d"]:
        if c in df_fc.columns:
...
def main():
    ...
    model_dir = "model"
    forecast = "forecast_input_2025-04-03.xlsx"
    date = "2025-04-03"
    out = "outPut/forcast_2025-04-03.xlsx"
