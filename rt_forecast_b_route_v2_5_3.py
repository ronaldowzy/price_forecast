...
def compute_rt_state_for_date(df_hist: pd.DataFrame, target_date: pd.Timestamp) -> Tuple[pd.DataFrame, float]:
    ...
    # Choose the most recent *complete* historical day as the "lag-1d" anchor.
    # This avoids accidentally using an in-progress day (e.g., today) that has only partial RT points.
    # A "complete day" here means we have at least 96 distinct slot_id values with non-null RT.
    day_slot_cnt = (
        hist.groupby("date_dt")["slot_id"]
        .nunique()
        .sort_index()
    )
    complete_days = day_slot_cnt[day_slot_cnt >= 96]
    if len(complete_days) > 0:
        d1 = complete_days.index.max()
    else:
        # fallback: use the most recent day with sufficient coverage (>=80 points) if available
        partial_days = day_slot_cnt[day_slot_cnt >= 80]
        d1 = partial_days.index.max() if len(partial_days) > 0 else (hist["date_dt"].max() if len(hist) > 0 else target_date - pd.Timedelta(days=1))

    # Keep the relative gap between lag_1d and lag_7d consistent with the original design:
    # (target-1) - (target-7) = 6 days, so we set lag_7d = d1 - 6 days.
    d7 = d1 - pd.Timedelta(days=6)
