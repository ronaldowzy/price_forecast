...
# ------------------------------
# [时段映射] slot_id_from_time(time_str)
# 口径：
#   以“15min区间结束时刻”作为标签：00:15→1 ... 23:45→95, 24:00→96。
#   这一口径会贯穿：数据对齐、rolling画像、预测输出、以及交易简表的时段解释。
# ------------------------------
...
# ------------------------------
# [日期/时刻标准化] add_slot_calendar(df)
# 业务含义：
#   将原始表中的“日期 + 时刻”统一映射到：
#     - date_dt：日期（datetime）
#     - slot_id：1..96 的 15min 时段编号（区间结束标签）
#   并补充日历特征（节假日/周末/月/季节），作为所有模型的基础输入。
# ------------------------------
...
# ------------------------------
# [净负荷口径] add_netload(df, is_history)
# 业务含义：
#   构造用于价格解释的“供需紧张度”核心指标 NetLoad（净负荷）：
#     NetLoad = 负荷 - 风 - 光 - 受电 -（可选：非市场核电/自备机组/地方电厂等供给）
#
# 为什么要同时算预测与实际：
#   - 训练阶段需要 NetLoad_act 与 NetLoad_fc 才能得到 dNetLoad = act - fc
#   - 预测阶段 only forecast：只需要 NetLoad_fc（以及其衍生 Ramp 特征）
#
# v2.6 扩展：
#   额外将 NonMarketNuclear / SelfSupplyUnits / LocalPowerPlant 纳入净负荷，
#   缺列默认 0（工程容错），但也意味着数据口径变动可能被“静默吞掉”。
# ------------------------------
...
# ------------------------------
# [有效净负荷] add_eff_netload_from_ps(df, ps_col)
# 业务含义：
#   将抽蓄以“等效负荷 PS_as_load”的方式并入净负荷，形成 NetLoad_eff：
#     NetLoad_eff = NetLoad_fc + PS_pred_as_load
#   并计算其爬坡 Ramp_eff，用于解释尖峰/负价更敏感的“变化率”触发。
#
# 注意：
#   - 这里使用的是“预测得到的 PS_pred_as_load”（不是实际PS）
#   - PS_pred_as_load = ps_alpha * ps_sign_as_load * PS_p50_pred
#     * ps_sign_as_load 口径在 infer_ps_sign_as_load() 固化
#     * ps_alpha 是强度折扣，可在线 override（不必重训）
# ------------------------------
def add_eff_netload_from_ps(df: pd.DataFrame, ps_as_load_col: str, out_prefix: str = "") -> pd.DataFrame:
    ...
    df[f"{out_prefix}NetLoad_eff_fc"] = pd.to_numeric(df["NetLoad_fc"], errors="coerce") + pd.to_numeric(
        df[ps_as_load_col], errors="coerce")
...
# ------------------------------
# [误差画像特征] add_error_profile_features(df)
# 业务含义：
#   为每个 slot_id（同一日内15min位置）刻画“净负荷预测误差 dNetLoad 的惯性与分布”。
#   这是 B1/PS 模型能够利用“近期系统性偏差/波动”进行自适应的核心特征来源。
#
# 核心做法：
#   - 沿 date_dt 方向做 rolling，但按 slot_id 分组（保证同一时刻位置跨日可比）
#   - 先 shift(1)：严格使用“过去信息”（D-1及更早），避免同日泄漏
#   - 再计算窗口统计（均值/绝对均值/分位数等）
#
# 产出：
#   Err_1:      前一日同slot误差
#   Err_mean_7: 过去7日均值（min_periods=5）
#   Err_absmean_7: 过去7日绝对误差均值
#   Err_p10_14 / Err_p90_14: 过去14日误差分位数（min_periods=10）
# ------------------------------
...
# ------------------------------
# [RT状态特征] add_rt_lag_features(df)
# 业务含义：
#   把“市场价格状态”作为模型输入的一部分，主要服务于：
#     - PS 模型：抽蓄行为与价格状态强相关（调度/套利/系统平衡）
#     - B2 模型：在同样的物理负荷条件下，价格体制也会随市场状态变化
#
# 核心做法：
#   - 按 slot_id 分组，对 RT 做跨日滞后：RT_lag_1d / RT_lag_7d
#   - 计算“前一日的日内波动”RT_vol_1d（按天 std 后再 shift(1)）
#     * 注意：RT_vol_1d 是 day-level 特征，回填到该日所有 96 个点
# ------------------------------
...
# ------------------------------
# [防泄漏关键] b1_oof_predictions(df_train, n_splits)
# 业务含义：
#   训练 B2 之前，先用时间序列切分生成 B1 的 out-of-fold(OOF) 预测：
#     dNet_p10_oof / dNet_p50_oof / dNet_p90_oof
#
# 为什么必须 OOF：
#   B2 的输入包含“净负荷误差的预测分位数”。
#   若用同一批样本“训练B1并预测自身”，B2 会看到过于理想的特征，回测虚高，上线崩塌。
#
# 实现要点：
#   - 以“天”为单位 TimeSeriesSplit，保证训练折永远早于验证折
#   - 样本天数太少时可能退化为全量拟合（需要在评估报告中显式标注）
# ------------------------------
...
# ------------------------------
# [抽蓄符号口径] infer_ps_sign_as_load(df_train)
# 业务含义：
#   固化“抽蓄原始数据的符号含义”与“等效负荷表达”的转换方向。
#
# 本脚本的约定：
#   抽蓄(实际) > 0 表示发电（增加供给，压低价格）
#   抽蓄(实际) < 0 表示抽水（增加负荷，抬高价格）
#   因此：PS_as_load = -PS_act  → ps_sign_as_load = -1（固定）
#
# 这是高杠杆假设：
#   若数据源符号口径反了，会系统性扭曲 NetLoad_eff 与 B2 结果。
# ------------------------------
...
# ------------------------------
# [防泄漏关键] ps_oof_predictions(df_oof, n_splits)
# 业务含义：
#   类似 B1 OOF，对抽蓄中位数 PS_p50 生成 OOF 预测列 PS_p50_oof，
#   供后续构造 NetLoad_eff 并训练 B2。
#
# 价值：
#   抽蓄对价格体制的影响很强；不用 OOF 会显著高估 B2 在回测期的表现。
# ------------------------------
...
# ------------------------------
# [价格体制模型] train_b2_regime_models(df_b2, th_spike, th_neg)
# 业务含义：
#   将价格分解为“体制(regime) + 幅度(magnitude)”：
#     1) 负价体制：P_neg = P(RT < th_neg)
#     2) 尖峰体制：P_spike = P(RT > th_spike)
#     3) 常态体制：其余区间
#   并分别训练：
#     - 两个分类器（spike/neg）+ 可选概率校准器（isotonic）
#     - 三套回归器：常态分位数回归 + 尖峰幅度回归 + 负价幅度回归
#
# 注意：
#   尖峰/负价幅度回归往往样本稀少；脚本对样本数过小会跳过训练（回退策略）。
# ------------------------------
...
# ==============================
# [特征底座构建] build_history_feature_frame(history_path)
# 目的：
#   将“历史全量数据（含预测+实际+RT+抽蓄）”统一加工成可训练/可预测共用的特征帧 df_feat。
#   这一步不是训练模型，而是把后续 B1/PS/B2 训练与 D+1 预测所需的特征都一次性算齐。
#
# 关键输出列（按模块）：
#   1) 时段体系：date_dt + slot_id(1..96, 15min区间结束时刻标签)
#   2) 物理量：NetLoad_fc / NetLoad_act / dNetLoad（净负荷误差）
#   3) 抽蓄：PS_act（仅读入/裁剪，不在此处确定“等效负荷符号”）
#   4) 日内变化：Ramp_fc_1 / Ramp_fc_4 / dPV_fc_1
#   5) 误差画像（跨日 rolling）：Err_1 / Err_mean_7 / Err_absmean_7 / Err_p10_14 / Err_p90_14
#   6) 市场状态（跨日 lag + 日内波动）：RT_lag_1d / RT_lag_7d / RT_vol_1d
#
# 为什么这一步对“月更模型、日更数据”很重要：
#   D+1 预测时，模型参数（joblib bundle）可能一个月才更新；
#   但这里的 Err_* 与 RT_* 特征是每日随最新真实数据滚动更新的，
#   属于对漂移的“在线自适应”部分，可显著缓冲模型陈旧带来的退化。
# ==============================
...
# ==============================
# [训练入口] train_and_save(history_path, model_dir, th_spike, th_neg, ps_alpha=0.6)
# 目标：
#   训练并保存一个可直接用于 D+1 预测的模型 bundle（joblib），并输出 meta.json 便于追溯。
#
# 训练流程（高层）：
#   1) build_history_feature_frame：构建历史特征底座 df_feat
#   2) select_train_rows：筛选训练样本 df_train
#   3) 生成 OOF：b1_oof_predictions + ps_oof_predictions
#      - 用 OOF 预测构造 df_b2（避免 B2 输入泄漏）
#   4) 全量训练：train_b1_models / train_ps_models / train_b2_regime_models
#   5) 打包落盘：阈值、特征列清单、抽蓄口径参数、模型对象、诊断信息等
#
# 产出：
#   - rt_routeB_v2_6.joblib：预测时唯一需要加载的“模型包”
#   - rt_routeB_v2_6_meta.json：便于人工审阅与追踪训练数据来源/口径
# ==============================
...
# ------------------------------
# [预测期画像] compute_error_profile_for_date(df_hist, target_date)
# 业务含义：
#   在 D+1 预测时，从“D之前的历史特征帧”中提取/计算“用于D日的误差画像特征 Err_*”。
#   这一步将每日更新的历史数据转换成当日可用的“在线自适应”特征，缓冲模型陈旧。
# ------------------------------
def compute_error_profile_for_date(df_hist: pd.DataFrame, target_date: dt.date) -> pd.DataFrame:
    ...
    def _slot_stats(g: pd.DataFrame) -> pd.Series:
        ...
        if len(s) == 0:
            return pd.Series(
                {"Err_1": 0.0, "Err_mean_7": 0.0, "Err_absmean_7": 0.0, "Err_p10_14": 0.0, "Err_p90_14": 0.0})
...
# ------------------------------
# [预测期市场状态] compute_rt_state_for_date(df_hist, target_date)
# 业务含义：
#   在 D+1 预测时，为 D 日构造 RT_lag_1d/7d（按slot）以及 RT_vol_1d（按天）。
#   它们刻画了“市场状态/波动水平”，影响抽蓄预测与价格体制判断。
# ------------------------------
...
# ==============================
# [预测入口] predict_one_day(history_path, model_dir, forecast_path, date_str, ...)
# 目标：
#   对目标日 D（date_str）的 96 个 15-min 时段输出 RT 价格预测（含概率/分位数/门控结果），
#   并可选导出交易简表（brief）用于交易决策展示。
#
# 输入拆分（非常重要）：
#   A) forecast_path：D日的“D+1预测输入”（只含预测侧字段）→ 形成 NetLoad_fc/Ramp_fc 等基础特征
#   B) history_path：D之前的“历史全量”（含实际与RT）→ 形成 Err_* 与 RT_* 这类滚动画像/状态特征
#   C) model_dir：加载 rt_routeB_v2_6.joblib → 模型参数/阈值/特征列清单/抽蓄口径等
#
# 预测主流程（高层）：
#   1) 读 forecast 并映射到 slot_id（强制 96 点完整）
#   2) 用 history 构造 Err_* 与 RT_* 并 merge 到 D 日特征
#   3) B1：预测 dNetLoad 分位数 → 推断 NetLoad_act_pXX
#   4) PS：预测抽蓄 → 得到 PS_pred_as_load → 构造 NetLoad_eff 与 Ramp_eff
#   5) B2：分类得 P_neg/P_spike（可校准）+ 三情景回归得幅度/分位数
#   6) 概率/情景组合（mixture/gate）+ spike budget 修正 → 形成最终可交易曲线
#
# 输出：
#   - 逐点预测列（p_neg/p_spike、P10/P50/P90、gate结果等）
#   - 可选：交易简表（Excel + 图表页）
# ==============================
def predict_one_day(
        history_path: str,
    ...
    df_out = pd.DataFrame({
        ...
        "p_full_neg_l10_eff": np.full_like(p_neg,
                                           float(p_full_neg_l10_eff) if p_full_neg_l10_eff is not None else np.nan,
                                           dtype=float),
...
# ------------------------------
# [尖峰预算] select_spike_budget_trigger(...)
# 业务含义：
#   在预测尖峰时段时，限制每天“触发尖峰修正/加仓信号”的时段数量与相邻间距，
#   使策略更符合交易执行能力与风险预算（避免全天乱跳）。
#
# 典型副作用排查：
#   若发现“买入机会总集中在 20-22 点”，需要审计：
#     - score_mode（以谁为分数？p_spike 还是幅度？）
#     - min_gap/block_radius 的抑制逻辑是否过强
#     - 是否某些特征（如 Ramp_eff）在晚高峰系统性偏大导致排序偏置
# ------------------------------
...
def _make_trading_brief_chart_sheet(
    ...
    try:
        ...
        spike_flag = pd.to_numeric(df[col_spike_flag], errors="coerce").fillna(0).astype(
            int).values if col_spike_flag else None
        ...
        if np.isfinite(p50).any():
            ...
            plt.annotate(f"最高 {p50[imax]:.1f}\n{times[imax]}", (imax, p50[imax]), textcoords="offset points",
                         xytext=(10, 10))
            plt.annotate(f"最低 {p50[imin]:.1f}\n{times[imin]}", (imin, p50[imin]), textcoords="offset points",
                         xytext=(10, -30))
        ...
        max_val = float(np.nanmax(p50));
        max_t = times[int(np.nanargmax(p50))]
        min_val = float(np.nanmin(p50));
        min_t = times[int(np.nanargmin(p50))]

        ws_chart["A2"] = f"P50 最高值：{max_val:.1f} 元/MWh（{max_t}）；最低值：{min_val:.1f} 元/MWh（{min_t}）"
        ws_chart[
            "A3"] = f"负价点数：{neg_cnt} / {len(p50)}（约 {neg_cnt / 4:.2f} 小时）；尖峰标记点数：{spike_cnt} / {len(p50)}"
...
# ------------------------------
# [交易简表输出] export_trading_brief_excel(...)
# 业务含义：
#   将预测结果整理为交易员/管理层可直接阅读的Excel：
#     - 明细表（每15min的关键字段）
#     - 图表页（含负价/尖峰标记、阈值线、预测分位数带等）
#   这部分不影响预测数值，但影响“可解释性与可执行性”。
# ------------------------------
...
# [回测入口] backtest_dates(...)
# 业务含义：
#   对多个日期循环调用 predict_one_day，并计算预测误差、分类效果、以及可选交易相关指标。
#   用于：
#     - 对比不同阈值/ps_alpha/gate参数
#     - 检验OOF与概率校准的稳定性
# ------------------------------
def backtest_dates(
        history_path: str,
    ...
    for d in dates:
        ...
        spike_selected_n = int(pd.to_numeric(df.get("spike_selected"), errors="coerce").fillna(0).astype(
            int).sum()) if "spike_selected" in df.columns else 0
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as w:
        ...
        try:
            run_info = {
                ...
                "ps_alpha_used": (float(ps_alpha_override) if ps_alpha_override is not None else (
                    float(ps_alpha_bundle) if ps_alpha_bundle == ps_alpha_bundle else np.nan)),
                "p_gate_spike": float(p_gate_spike),
                "p_gate_neg": float(p_gate_neg),
...
def parse_args() -> argparse.Namespace:
    ...
    p_train.add_argument("--th_spike", type=float, default=SPIKE_THRESHOLD_RT_DEFAULT,
                         help="Spike threshold (default 800)")
    p_train.add_argument("--th_neg", type=float, default=NEG_THRESHOLD_RT_DEFAULT,
                         help="Negative threshold (default 0)")
    p_train.add_argument("--ps_alpha", type=float, default=0.6,
                         help="Damping for pumped-storage effect (0..1). Use <1 to reduce noise.")
    ...
    p_pred.add_argument("--ps_alpha_override", type=float, default=None,
                        help="Override PS damping ps_alpha during prediction/backtest (no retraining).")
    p_pred.add_argument("--p_full_neg", type=float, default=P_FULL_NEG_DEFAULT,
                        help="Ramp gate: full negative weight threshold (<= p_gate_neg disables ramp)")
    p_pred.add_argument("--arm_day_neg", type=float, default=ARM_DAY_NEG_DEFAULT,
                        help="Day-level arming threshold for negative gate (<=0 disables)")
    p_pred.add_argument("--arm_day_neg_pv_th", type=float, default=ARM_DAY_NEG_PV_TH_DEFAULT,
                        help="PV_fc daily max threshold for negative-day arming guard")
    p_pred.add_argument("--arm_day_neg_netload_min_th", type=float, default=ARM_DAY_NEG_NETLOAD_MIN_TH_DEFAULT,
                        help="NetLoad_fc daily min threshold for negative-day arming guard")
    p_pred.add_argument("--arm_day_neg_use_or", type=int, default=1,
                        help="Use OR guard (1) or AND guard (0) for (PV vs NetLoad)")
    ...
    # v2.5 spike budget trigger
    p_pred.add_argument("--spike_budget_enable", type=int, default=int(SPIKE_BUDGET_ENABLE_DEFAULT),
                        help="Enable spike budget trigger (1/0)")
    p_pred.add_argument("--spike_score_mode", type=str, default=SPIKE_SCORE_MODE_DEFAULT,
                        choices=["p_spike", "u90", "hybrid"],
                        help="Spike budget scoring base: p_spike (legacy), u90 (rank(RT_norm_p90)), hybrid (max(rank(p_spike),rank(u90)))")
    p_pred.add_argument("--spike_budget_pts", type=int, default=SPIKE_BUDGET_PTS_DEFAULT,
                        help="Max number of 15-min slots flagged as spike per day")
    p_pred.add_argument("--spike_budget_seeds", type=int, default=SPIKE_BUDGET_SEEDS_DEFAULT,
                        help="Number of spike seed slots before expansion")
    p_pred.add_argument("--spike_budget_block_radius", type=int, default=SPIKE_BUDGET_BLOCK_RADIUS_DEFAULT,
                        help="Expand each seed by +/- radius (15-min slots)")
    p_pred.add_argument("--spike_budget_min_gap", type=int, default=SPIKE_BUDGET_MIN_GAP_DEFAULT,
                        help="Minimum separation between spike seeds (15-min slots)")
    p_pred.add_argument("--spike_apply_to_p50", type=int, default=int(SPIKE_APPLY_TO_P50_DEFAULT),
                        help="If 1, spike budget affects RT_gate_p50; if 0, only affects risk bounds & scenario columns")

    # trading brief (two-file output)
    p_pred.add_argument("--trading_brief", type=int, default=1,
                        help="If 1, also export a simplified trading brief Excel")
    p_pred.add_argument("--trading_out", default="",
                        help="Optional trading brief output path (default derived from --out)")
    ...
    p_bt.add_argument("--ps_alpha_override", type=float, default=None,
                      help="Override PS damping ps_alpha during backtest (no retraining).")
    ...
    p_bt.add_argument("--arm_day_neg", type=float, default=ARM_DAY_NEG_DEFAULT,
                      help="Day-level arming threshold for negative gate (<=0 disables)")
    p_bt.add_argument("--arm_day_neg_pv_th", type=float, default=ARM_DAY_NEG_PV_TH_DEFAULT,
                      help="PV_fc daily max threshold for negative-day arming guard")
    p_bt.add_argument("--arm_day_neg_netload_min_th", type=float, default=ARM_DAY_NEG_NETLOAD_MIN_TH_DEFAULT,
                      help="NetLoad_fc daily min threshold for negative-day arming guard")
    p_bt.add_argument("--arm_day_neg_use_or", type=int, default=1,
                      help="Use OR guard (1) or AND guard (0) for (PV vs NetLoad)")
    ...
    # v2.5 spike budget trigger
    p_bt.add_argument("--spike_budget_enable", type=int, default=int(SPIKE_BUDGET_ENABLE_DEFAULT),
                      help="Enable spike budget trigger (1/0)")
    p_bt.add_argument("--spike_score_mode", type=str, default=SPIKE_SCORE_MODE_DEFAULT,
                      choices=["p_spike", "u90", "hybrid"],
                      help="Spike budget scoring base: p_spike (legacy), u90 (rank(RT_norm_p90)), hybrid (max(rank(p_spike),rank(u90)))")
    p_bt.add_argument("--spike_budget_pts", type=int, default=SPIKE_BUDGET_PTS_DEFAULT,
                      help="Max number of 15-min slots flagged as spike per day")
    p_bt.add_argument("--spike_budget_seeds", type=int, default=SPIKE_BUDGET_SEEDS_DEFAULT,
                      help="Number of spike seed slots before expansion")
    p_bt.add_argument("--spike_budget_block_radius", type=int, default=SPIKE_BUDGET_BLOCK_RADIUS_DEFAULT,
                      help="Expand each seed by +/- radius (15-min slots)")
    p_bt.add_argument("--spike_budget_min_gap", type=int, default=SPIKE_BUDGET_MIN_GAP_DEFAULT,
                      help="Minimum separation between spike seeds (15-min slots)")
    p_bt.add_argument("--spike_apply_to_p50", type=int, default=int(SPIKE_APPLY_TO_P50_DEFAULT),
                      help="If 1, spike budget affects RT_gate_p50; if 0, only affects risk bounds & scenario columns")
...
def run_without_args() -> None:
    CONFIG = {
        ...
        # predict
        "forecast": r"forecast_input_2026-01-15.xlsx",
        "date": "2026-01-15",
        "out": r"outPut/forcast_2026-01-15.xlsx",
        "trading_brief": 1,
        "trading_out": r"",
    ...
    if CONFIG["cmd"] == "train":
        train_and_save(CONFIG["history"], CONFIG["model_dir"], th_spike=float(CONFIG["th_spike"]),
                       th_neg=float(CONFIG["th_neg"]), ps_alpha=float(CONFIG.get("ps_alpha", 0.6)))
        ...
        predict_one_day(
            ...
            arm_day_neg_netload_min_th=float(
                CONFIG.get("arm_day_neg_netload_min_th", ARM_DAY_NEG_NETLOAD_MIN_TH_DEFAULT)),
